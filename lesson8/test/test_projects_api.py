...
class TestYouGileProjects:
    ...
    def test_create_project_positive(self, base_url, headers):
        ...
        ), f"Не удалось получить проект: {get_response.text}"
        get_data = get_response.json().get("data") or get_response.json()

        assert (
            get_data.get("name") == expected_title
            or get_data.get("title") == expected_title
    ...
    def test_create_project_negative_empty_title(self, base_url, headers):
        ...
        assert (
            "title" in str(error_data).lower()
            or "required" in str(error_data).lower()
        )

    def test_get_project_positive(
        self, base_url, headers, create_temp_project
    ):
    ...
    def test_update_project_positive(
        ...
        assert (
            get_data.get("name") == new_title
            or get_data.get("title") == new_title
        ), f"Ожидалось название = {new_title}, получено: {get_data}"

    def test_update_project_negative_invalid_title(
        self, base_url, headers, create_temp_project
    ):
        ...
        print(
            f"Invalid update response: {response.status_code}, {response.text}"
        )
        ...
        if response.status_code == 400:
            ...
            ), "Title изменился на пустое значение!"
        else:
